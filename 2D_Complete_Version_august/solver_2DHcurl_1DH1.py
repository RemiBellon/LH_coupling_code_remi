...
class LHCouplingSolver_2DHcurl_1DH1:
    ...
    def build_mesh_with_PMLs(self) -> None:
        ...
        if self.box_medium == "VACUUM":
            ...
            if self.geom_mode == "1D":
                ...
                if self.antenna_grill is not None:
                    # --- EXPERT FIX: Rigid Topological Alignment ---
                    self.Lz_wall = self.cfg['DOMAIN'].get('Lz_wall', 0.02)
                    
                    # 1. Generate waveguides starting exactly after the bottom wall
                    self.instructions = self.antenna_grill.generate_mesh_instructions(z_start_position=self.Lz_wall)
                    
                    # 2. Extract the exact length of the antenna from the final coordinate
                    self.Lz_antenna = self.instructions[-1]['z_end'] - self.instructions[0]['z_start']
                    
                    # 3. The source plasma length IS the antenna length
                    self.cfg['DOMAIN']['Lz_plasma'] = self.Lz_plasma_src = self.Lz_antenna
                else:
        ...
        if self.Lx_wg > 0 and self.instructions:
            # Create a monolithic vacuum block for the entire antenna module
        ...
        elif self.geom_mode=="2D": # FULL_2D or VACUUM (with toroidal PMLs)
            if self.antenna_grill is None:
                ...
                if self.instructions:
                    ...
                    z_first = self.Lz_wall
                    z_last = self.Lz_wall + self.Lz_plasma_src
...
# =====================================================================
    def solve_helmholtz_2DHcurl_1DH1_with_pml(self, mesh, geom_mode, box_medium, p_order):
        ...
        for step in range(max_amr_steps):
            ...
            if self.geom_mode != "1D":
                ...
                # In 1D, toroidal standing waves cannot physically exist due to periodicity.
                Gamma_T = 0.0
                z_target_T = None
                peak_x_T = None
            ...
            if step > 0:
                ...
                if delta_gamma < tolerance:
                    print(f'delta_gamma: {delta_gamma:.3e} < tolerance: {tolerance:.3e}')
                    print("--- AMR Converged ---")
                    break
            ...
            marking_fraction = self.cfg['DOMAIN'].get('amr_marking_fraction', 0.7)
            
            # 5. Mark Elements and Refine
            max_err = max(elerr)
            for el in self.mesh.Elements():
                self.mesh.SetRefinementFlag(el, elerr[el.nr] > marking_fraction * max_err)
    ...
    def extract_tangential_aperture_fields(self, x_target=0.0, num_points=4000):
        ...
        Ex, Ey, Ez = [], [], []
        Hy, Hz = [], [] # We need the tangential H-fields for the Poynting spectrum
        
        # Reconstruct H field natively
        E_plane_gf, E_outplane_gf = self.E_field.components[0], self.E_field.components[1]
        curl_E_sol = CF(( -E_outplane_gf.Deriv()[1], -curl(E_plane_gf), E_outplane_gf.Deriv()[0] ))
        H_sol_3D = curl_E_sol / (1j * self.omega_LH * self.mu0)
        for z in z_sweep:
            try:
                ...
                Ex.append(self.E_field.components[0][0](mip)) 
                Ey.append(self.E_field.components[1](mip))    
                Ez.append(self.E_field.components[0][1](mip)) 
                Hy.append(H_sol_3D[1](mip))
                Hz.append(H_sol_3D[2](mip))
            except Exception:
                Ex.append(0j); Ey.append(0j); Ez.append(0j); Hy.append(0j); Hz.append(0j)
        ...
        field_data = {
            ...
            'Hy': np.array(Hy, dtype=complex), 'Hz': np.array(Hz, dtype=complex)
        }
        return field_data

    def compute_aloha_normalized_spectrum(self, field_data, P_coupled_net):
        """
        Computes the Spectral Active Power Density (dP/dn_para) 
        using Parseval's theorem on the tangential E and H fields.
        """
        print("\n--- Computing Rigorous Spectral Poynting Flux (ALOHA Benchmark) ---")

        z_sweep = field_data['z_coords']
        Ey, Ez = field_data['Ey'], field_data['Ez']
        Hy, Hz = field_data['Hy'], field_data['Hz']
        dz = z_sweep[1] - z_sweep[0]
        num_points = len(z_sweep)

        # Spatial FFT of tangential fields
        Ey_fft = np.fft.fftshift(np.fft.fft(Ey, 8*num_points)) * dz
        Ez_fft = np.fft.fftshift(np.fft.fft(Ez, 8*num_points)) * dz
        Hy_fft = np.fft.fftshift(np.fft.fft(Hy, 8*num_points)) * dz
        Hz_fft = np.fft.fftshift(np.fft.fft(Hz, 8*num_points)) * dz

        # Wavevector and Refractive Index Arrays
        k_z = np.fft.fftshift(np.fft.fftfreq(8*num_points, d=dz)) * 2.0 * np.pi
        n_para = k_z / self.k0

        # --- EXPERT FIX: True Spectral Poynting Flux ---
        # S_x(k_z) = 1/2 * Re( E_y * H_z^* - E_z * H_y^* )
        spectral_S_x = 0.5 * np.real(Ey_fft * np.conj(Hz_fft) - Ez_fft * np.conj(Hy_fft))
        
        # Scale by k0 / (2*pi) to convert dP/dk_z to dP/dn_para
        # This yields the exact physical watts per unit of n_parallel, perfectly matching ALOHA's dP_nz
        power_spectrum = spectral_S_x * (self.k0 / (2.0 * np.pi))
