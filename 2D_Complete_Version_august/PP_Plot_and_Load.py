...
def plot_2D_wave_map(h5_filepath, figure_save_dir, mode, component='Ez', value_type='real', antenna_grill=None, 
                     Lx_wg=0.08, Lz_wall=0.02, plot_poynting=True, show_windows_R=True, show_windows_T=True, Poynting_box=True):
    ...
    if show_windows_T and 'z_target_T' in diag_data_loaded: 
        dx_tor = 0.02 * Lx_plasma 
        z_target_T = diag_data_loaded.get('z_target_T')
        peak_x_T = diag_data_loaded.get('peak_x_T')
        
        # EXPERT FIX: Ensure coordinates are valid floats before drawing
        if z_target_T is not None and peak_x_T is not None and not np.isnan(z_target_T):
            ...
            z_start = z_target_T - window_size_toroidal if n_para >= 0 else z_target_T 
                
            rect_T = patches.Rectangle(
                (z_start, peak_x_T - dx_tor/2), 
                window_size_toroidal, dx_tor, 
                linewidth=2, edgecolor='magenta', facecolor='none', hatch='\\\\'
            )
            ax.add_patch(rect_T)
        else:
            print("--- Toroidal window data is None/NaN. Skipping overlay. ---")
