...
# ==========================================
# PIECEWISE PROFILE BUILDER
# ==========================================
class GaussianBlob(BaseModel):
    type: Literal["gaussian_blob"]
    amplitude: float
    x_c: float
    z_c: float
    sigma_x: float = Field(gt=0.0)
    sigma_z: float = Field(gt=0.0)
...
class PiecewiseProfile(BaseModel):
    ...
    segments: List[Literal["linear", "exponential", "constant", "quadratic"]]
...
class PlasmaConfig(BaseModel):  # Plasma properties sub-structure
    ...
    perturbations: Optional[List[GaussianBlob]] = Field(default_factory=list)

    @model_validator(mode='after')
    def validate_perturbations(self):
        # safety check: warn the user if they turned the toggle ON but forgot the blobs
        if self.use_perturbations and not self.perturbations:
            raise ValueError("use_perturbations is True, but the perturbations list is empty!")
        return self
...
class AntennaDimensions(BaseModel): # Antenna geometry dimensions sub-structure:
    Lx_wg_active: float = Field(ge=0.0)
    Lx_wg_passive: float = Field(ge=0.0)
    Ly_wg: float = Field(ge=0.0)
    Ly_wall: float = Field(ge=0.0)
    Lz_wg_active: float = Field(ge=0.0)
    Lz_wg_passive: float = Field(ge=0.0)
    Lz_septa: float = Field(ge=0.0)
    Lz_gap_module: float = Field(ge=0.0)
    # Lz_wall: float = Field(ge=0.0)
...
class AntennaArrangement(BaseModel):        # Antenna grill sub-structure
    ...
    active_waveguides_per_module_row: List[int]
    ...
    def validate_lists_match_modules(self):
        ...
        if len(self.active_waveguides_per_module_row) != n_mod:
            raise ValueError(f"'active_waveguides_per_module_row' list length must equal num_modules ({n_mod}).")
...
class AntennaConfig(BaseModel): # AntennaConfig main structure
    ...
    edge_passives: bool = False
    inter_module_passives: bool = False
    dimensions: AntennaDimensions
    grill_arrangement: AntennaArrangement

    @model_validator(mode='after')
    def enforce_pam_passives(self):
        # If PAM, passives are physically required. If FAM, user overrides apply.
        if self.topology == "PAM":
            self.edge_passives = True
            self.inter_module_passives = True
        return self

    @computed_field
    def total_width(self) -> float:
        """Calculates exact toroidal width dynamically for ANY configuration."""
        active_wgs_list = self.grill_arrangement.active_waveguides_per_module_row
        total_active_wgs = sum(active_wgs_list)

        total_passive_wgs = 0
        if self.edge_passives:
            total_passive_wgs += 2

        if self.inter_module_passives:
            total_passive_wgs += (self.grill_arrangement.num_modules - 1)

        # Intra-module passives strictly belong to the PAM definition
        if self.topology == "PAM":
            for n_active in active_wgs_list:
                total_passive_wgs += (n_active - 1)

        total_wgs = total_active_wgs + total_passive_wgs
        total_septa = total_wgs - 1 if total_wgs > 0 else 0

        return (total_active_wgs * self.dimensions.Lz_wg_active) + \
               (total_passive_wgs * self.dimensions.Lz_wg_passive) + \
               (total_septa * self.dimensions.Lz_septa)
...
class DomainConfig(BaseModel):
    ...
    def Lz_tot(self) -> float:
        ...
        if self.Lz_plasma is None:
            return 0.0
        return self.Lz_plasma + (2 * self.Lz_pml)
...
class SimulationConfig(BaseModel): # main structure
    ...
    def enforce_strict_physics_and_geometry(self):
        ...
        # --- Lock Lz_plasma to Antenna Width ---
        if antenna is not None:
            # Overwrite any user input with the exact mathematical width
            self.geometry.domain.Lz_plasma = antenna.total_width + (2 * domain.Lz_wall)
