...
class LHCouplingSolver_2DHcurl_1DH1:
 ...
 # --- 1. Extract Core Parameters ---
        ...
        if self.n_para != 0: 
            ...
            self.Lz_exact = 1.0 * lambda_para
            self.cfg['DOMAIN']['Lz_plasma'] = self.Lz_exact
        else:
            self.Lz_exact = self.cfg['DOMAIN']['Lz_plasma']

        # The mesh MUST resolve the smallest wavelength in the system (Slow Wave)
        lambda_min = lambda_perp_SW
        self.h_max = lambda_min / self.cfg['DOMAIN']['n_resol_per_wlgth']

        # --- 4. OCC Geometry Construction ---
        # Domain 1 : Plasma
        rect_plasma = occ.MoveTo(0, 0).Rectangle(self.Lx_plasma, self.Lz_exact).Face()
        ...
        # Domain 2 : PML
        rect_pml = occ.MoveTo(self.Lx_plasma, 0).Rectangle(self.Lx_pml, self.Lz_exact).Face()
...
# =====================================================================
    def solve_helmholtz_Hcurl_2D_pml(self, mesh, cfg):
        ...
        Sr_Real = self.cfg['PML'].get('S_real', 1.0)
        Sr_Imag = self.cfg['PML'].get('S_imag', 1.0)
        pr = self.cfg['PML'].get('p_degree', 2.0)
