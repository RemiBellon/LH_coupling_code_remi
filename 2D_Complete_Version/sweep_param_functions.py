...
def generate_2D_toroidal_pml_database(base_cfg, geom_mode, box_medium, save_dir="Results"):
    """
    Executes independent sweeps on Toroidal PML parameters 
    for several values of n_parallel and saves to an HDF5 database.
    Designed to evaluate Toroidal SWR (Gamma_T).
    """
    print("=========================================================")
    print("  GÉNÉRATION DATABASE : PML TOROÏDALE 2D (JACQUOT 2013)  ")
    print("=========================================================")
    
    if geom_mode == "1D":
        print("[!] WARNING: geom_mode is '1D'. Toroidal PMLs are inactive in 1D (periodic boundaries). Forcing '2D' for this sweep.")
        geom_mode = "2D"

    os.makedirs(save_dir, exist_ok=True)
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    h5_filename = os.path.join(save_dir, f"PML_Toroidal_2D_Database_{timestamp}.h5")
    
    # 1. n_parallel testing spectrum
    if box_medium == "VACUUM":
        n_para_list = [0.0] # Adjust as needed for pure TM propagation
    else: 
        n_para_arr_negative = np.linspace(-2.5, -1.5, 5)
        n_para_arr_positive = np.linspace(1.5, 2.5, 5)
        # n_para_list = np.concatenate((n_para_arr_negative, n_para_arr_positive))
        n_para_list = [2.0]
        print(f"n_para_list: {n_para_list}")
    
    # 2. Definition of independent Toroidal sweeps
    # Format -> 'Group_Name': (Values_to_test, 'PML_key')
    scans = {
        'Sweep_Sz_real': (np.linspace(0.5, 5.0, 40), 'Sz_r'),
        'Sweep_Sz_imag': (np.linspace(0.5, 12.0, 40), 'Sz_im'),
        'Sweep_Resolution_PPW': (np.linspace(5.0, 60.0, 40), 'ppw_pml'), # Contrôle maxh_pml
        'Sweep_Resolution_PPW_Medium': (np.linspace(1.0, 40.0, 40), 'ppw_medium'), # Contrôle maxh_medium
        'Sweep_pz': (np.linspace(1.0, 5.0, 40), 'pz'),
        'Sweep_Lz_pml_ratio': (np.linspace(0.1, 4.5, 40), 'Lz_pml_ratio'), # Lz_pml / lambda_para
    }
    
    with h5py.File(h5_filename, 'w') as h5f:
        h5f.attrs['Description'] = "Validation PML 2D Toroidale (Jacquot 2013)"
        ...
        for group_name, (param_values, param_key) in scans.items():
            ...
            for n_para_val in n_para_list:
                # Bypass n_para=0 for plasma (avoid division by zero in lambda_para)
                if abs(n_para_val) < 1e-3 and box_medium != "VACUUM":
                    continue
                    
                print(f"  >> Test pour n_para = {n_para_val:.2f}")
                subgrp = grp.create_group(f"n_para_{n_para_val:.2f}")
                ...
                for val in param_values:
                    ...
                    if param_key in ['Sz_r', 'Sz_im', 'pz', 'ppw_pml']:
                        ...
                        # Convert ratio to physical Lz_pml distance based on parallel wavelength
                        lambda_para = scan_cfg['WAVE']['lambda0'] / max(abs(n_para_val), 1e-6)
                    ...
                    try:
                        ...
                        # Store Gamma reflections
                        eta_sim_R_list.append(diag_data['eta_sim_R'])
                        eta_pred_R_list.append(diag_data['eta_pred_R'])
                        eta_sim_T_list.append(diag_data['eta_sim_T'])
                        eta_pred_T_list.append(diag_data['eta_pred_T'])
                        ...
                        current_Lz_pml = scan_cfg['DOMAIN'].get('Lz_pml', 0.0)
                        Lz_pml_list.append(current_Lz_pml)
                        
                        print(f'     [OK] {param_key}={val:.3f} | Gamma_T={diag_data["eta_sim_T"]:.2e} | Lz_pml={current_Lz_pml:.4f}m')
                ...
                # Save datasets to HDF5 subgroup
                subgrp.create_dataset('dofs', data=dofs_list)
                
                # Note: We save Gamma_T into the 'eta_sim' key so your generic plotting functions 
                # (like `plot_concatenated_general_sweeps`) automatically plot the Toroidal reflection.
                subgrp.create_dataset('eta_sim', data=eta_sim_T_list) 
                subgrp.create_dataset('eta_pred', data=eta_pred_T_list)
                
                # Keep Radial data for reference if needed
                subgrp.create_dataset('eta_sim_R_ref', data=eta_sim_R_list) 
                
                subgrp.create_dataset('conservation_error', data=conservation_err_list)
                subgrp.create_dataset('fraction_radial', data=fraction_radial_list)
                subgrp.create_dataset('fraction_toroidal', data=fraction_toroidal_list)
                subgrp.create_dataset('Lz_pml', data=Lz_pml_list)
...
FORMAT_LABELS = {
    ...
    'Sz_im': "$S_z^{im}$",
    'pz': "$p_z$",
    'Lz_pml_ratio': "$L_{z,pml} / \lambda_\parallel$"
}
...
def plot_sweeps_all_npara(h5_filepath):
    ...
    with h5py.File(h5_filepath, 'r') as h5f:
        ...
        for sweep_name in sweep_groups:
            ...
            save_name = f"Analyse_Spectrale_{sweep_name}.pdf"
