...
class AntennaGrill:
    def __init__(self, Lx_wg_active: float, Lx_wg_passive: float, Ly_wg: float,
                 Lz_wg_active: float, Lz_wg_passive: float, Lz_septa: float,
                 Lz_gap: float, add_inter_module_passives: bool = False):
        """
        Lz_wg_active: Width of active waveguides (m)
        Lz_septa: Thickness of internal metallic septa (m)
        Lz_gap: Thickness between modules (m)
        Lx_wg_active: Depth of the active waveguides (m)
        Lx_wg_passive: Depth of the short-circuited passive waveguides (m)
        Lz_wg__passive: Width of passive waveguides (defaults to Lz_wg_active)
        Ly_wg: Poloidal height of the waveguide (m) - Required to map 3D Power to 2D Field
        """
        self.Lz_wg_active = Lz_wg_active
        self.Lz_septa = Lz_septa
        self.Lz_gap = Lz_gap
        self.Lx_wg_active = Lx_wg_active
        self.Lx_wg_passive = Lx_wg_passive if Lx_wg_passive is not None else Lx_wg_active
        self.Lz_wg_passive = Lz_wg_passive if Lz_wg_passive is not None else Lz_wg_active
        self.Ly_wg = Ly_wg

        self.add_inter_module_passives = add_inter_module_passives
        self.modules_config = []
    ...
    def generate_mesh_instructions(self, z_start_position: float, add_global_edge_passives: bool = True) -> List[Dict]:
        """
        ...
        if add_global_edge_passives:
            add_wg('wg_passive', self.Lz_wg_passive, self.Lx_wg_passive, 0j)
            add_wg('metal', self.Lz_septa, 0.0, 0j)
        ...
        for mod_idx, mod in enumerate(self.modules_config):
            ...
            for i in range(mod['num_active']):
                ...
                E_amp = np.sqrt((2.0 * Z0 * P_2D_wg) / self.Lz_wg_active)

                # C. Place Active Waveguide
                E_val = E_amp * np.exp(1j * total_phase_rad)
                add_wg('wg_active', self.Lz_wg_active, self.Lx_wg_active, E_val)
            ...
            if mod_idx < len(self.modules_config) - 1:
                add_wg('metal', self.Lz_septa, 0.0, 0j)
                if self.add_inter_module_passives:
                    add_wg('wg_passive', self.Lz_wg_passive, self.Lx_wg_passive, 0j)
                    add_wg('metal', self.Lz_septa, 0.0, 0j)
...
def build_grill_from_config(ant_cfg, dom_cfg):
    """ Centralized factory to build the antenna and generate instructions. """
    grill = AntennaGrill(
        Lx_wg_active=ant_cfg.dimensions.Lx_wg_active,
        Lx_wg_passive=ant_cfg.dimensions.Lx_wg_passive,
        Ly_wg=ant_cfg.dimensions.Ly_wg,
        Lz_wg_active=ant_cfg.dimensions.Lz_wg_active,
        Lz_wg_passive=ant_cfg.dimensions.Lz_wg_passive,
        Lz_septa=ant_cfg.dimensions.Lz_septa,
        Lz_gap=ant_cfg.dimensions.Lz_gap_module,
        add_inter_module_passives=ant_cfg.inter_module_passives)
    is_pam = (ant_cfg.topology == "PAM")
    for i in range(ant_cfg.grill_arrangement.num_modules):
        grill.add_module(
            num_active=ant_cfg.grill_arrangement.active_waveguides_per_module_row[i],
            is_PAM=is_pam,
            delta_phi_deg=ant_cfg.grill_arrangement.phase_shift_per_module_deg[i],
            power_module_W=ant_cfg.grill_arrangement.power_per_module_W[i])

    instructions = grill.generate_mesh_instructions(
        z_start_position=dom_cfg.Lz_wall,
        add_global_edge_passives=ant_cfg.edge_passives)
    return grill, instructions
